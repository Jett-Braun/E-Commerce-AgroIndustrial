...
MODEL_FILE = BASE_DIR / "model_cafe.pkl"
...
SUPABASE_MODEL_NAME = os.getenv("MODEL_NAME", "model_cafe.pkl")
...
def download_model_from_supabase() -> bool:
    ...
    try:
        ...
        with open(MODEL_FILE, "wb") as f:
            f.write(data)
        print(f"📥 [Café] Modelo {SUPABASE_MODEL_NAME} descargado exitosamente desde Supabase.")
        return True
    except Exception as e:
        print(f"ℹ️ [Café] No se pudo descargar el modelo ({e}). Se procederá a entrenar.")
        return False
...
def upload_model_to_supabase() -> bool:
    ...
    try:
        ...
        try:
            ...
            print(f"⚠️ [Café] Error al subir, intentando actualizar: {upload_error}")
        ...
        print(f"📤 [Café] Modelo {SUPABASE_MODEL_NAME} subido/actualizado exitosamente.")
        return True
    except Exception as e:
        print(f"❌ [Café] Error al subir el modelo: {e}")
        return False
...
def validate_csv_file():
    ...
    required_columns = ['service_id', 'batch_volume_qq', 'fuel_cost_per_liter', 'price']
    ...
    if (data['batch_volume_qq'] <= 0).any():
        raise CSVInvalidError("El volumen del lote debe ser mayor a 0.")
    ...
    # 9. Verificar que los service_id son válidos (1, 2, 3)
    valid_services = [1, 2, 3]
    invalid_services = data[~data['service_id'].isin(valid_services)]['service_id'].unique()
    if len(invalid_services) > 0:
        raise CSVInvalidError(
            f"Service IDs inválidos: {list(invalid_services)}. "
            f"Service IDs válidos: {valid_services}"
        )
...
def train_model_from_csv():
    ...
    try:
        print("📊 [Café] Iniciando entrenamiento...")
        
        # 1. Validar CSV
        data = validate_csv_file()
        
        # 2. Preparar características
        features = ['service_id', 'batch_volume_qq', 'fuel_cost_per_liter']
        
        # Agregar factor de mantenimiento si existe
        if 'mantenimiento_factor' in data.columns:
            features.append('mantenimiento_factor')
            X = data[features]
            print("📊 [Café] Usando mantenimiento_factor del CSV")
        else:
            # Si no existe, usar el valor por defecto
            data['mantenimiento_factor'] = latest_maint_factor
            features.append('mantenimiento_factor')
            X = data[features]
            print(f"📊 [Café] Usando mantenimiento_factor por defecto: {latest_maint_factor}")
        
        y = data['price']
        
        # 3. Entrenar el modelo
        print(f"🧠 [Café] Entrenando modelo con {len(X)} registros...")
        ...
        print(f"✅ [Café] Modelo entrenado: R²={r2:.4f}, MSE={mse:.4f}")
        
        # 5. Guardar el modelo
        with open(MODEL_FILE, "wb") as f:
            pickle.dump(new_model, f)
        print(f"💾 [Café] Modelo guardado en {MODEL_FILE}")
        ...
        try:
            ...
            print(f"⚠️ [Café] No se pudo subir a Supabase: {e}")
        ...
        model_loaded = False
        last_error = str(e)
        print(f"❌ [Café] Error: {e}")
        raise e
        
    except Exception as e:
        model_loaded = False
        last_error = str(e)
        print(f"❌ [Café] Error inesperado: {e}")
...
def load_or_train_model():
    ...
    print("🚀 [Café] Iniciando carga de modelo...")
    ...
    if download_model_from_supabase() and MODEL_FILE.exists():
        try:
            ...
            model_loaded = True
            last_error = None
            print("✅ [Café] Modelo cargado desde Supabase")
            return True
        except Exception as e:
            print(f"⚠️ [Café] Error al cargar modelo descargado: {e}")
    ...
    if MODEL_FILE.exists():
        try:
            ...
            model_loaded = True
            last_error = None
            print("✅ [Café] Modelo cargado desde archivo local")
            return True
        except Exception as e:
            print(f"⚠️ [Café] Modelo local corrupto: {e}")
    
    # 3. Entrenar desde CSV
    try:
        print("🔄 [Café] Entrenando modelo desde CSV...")
        result = train_model_from_csv()
        if result['status'] == 'success':
            print("✅ [Café] Modelo entrenado exitosamente")
            return True
    except Exception as e:
        last_error = str(e)
        print(f"❌ [Café] Error al entrenar: {e}")
        return False
...
async def quote_cafe(data: CafeRequest):
    ...
    try:
        # 2. Validar entrada
        if data.service_id not in [1, 2, 3]:
            raise HTTPException(
                status_code=400,
                detail=f"Tipo de servicio inválido: {data.service_id}. Tipos válidos: 1, 2, 3"
            )
        
        if data.batch_volume_qq <= 0:
            raise HTTPException(
                status_code=400,
                detail="El volumen del lote debe ser mayor a 0"
            )
        ...
        print(f"❌ [Café] Error en quote_cafe: {e}")
        print(traceback.format_exc())
        raise HTTPException(
            status_code=500,
...
print("=" * 60)
print("🚀 INICIANDO API DE CAFÉ (C1)")
print("=" * 60)
...
print("  - POST /api/cafe/quote")
print("  - POST /api/cafe/retrain")