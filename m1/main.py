...
# ============================================
# RUTAS ABSOLUTAS (SOLUCIONA EL ERROR DE LECTURA DEL CSV EN RENDER)
# ============================================
...
SUPABASE_MODEL_NAME = os.getenv("MODEL_NAME", "model_maquinaria.pkl")
...
def download_model_from_supabase() -> bool:
    ...
    try:
        ...
        with open(MODEL_FILE, "wb") as f:
            f.write(data)
        print(f"📥 [Maquinaria] Modelo {SUPABASE_MODEL_NAME} descargado exitosamente desde Supabase.")
        return True
    except Exception as e:
        print(f"ℹ️ [Maquinaria] No se pudo descargar el modelo ({e}). Se procederá a entrenar.")
        return False
...
def upload_model_to_supabase() -> bool:
    ...
    try:
        ...
        try:
            ...
            print(f"⚠️ [Maquinaria] Error al subir, intentando actualizar: {upload_error}")
        ...
        print(f"📤 [Maquinaria] Modelo {SUPABASE_MODEL_NAME} subido/actualizado exitosamente.")
        return True
    except Exception as e:
        print(f"❌ [Maquinaria] Error al subir el modelo: {e}")
        return False
...
def validate_csv_file():
    ...
    required_columns = ['equipment_type', 'hours_requested', 'fuel_cost_per_liter', 'price']
    ...
    if (data['hours_requested'] <= 0).any():
        raise CSVInvalidError("Las horas solicitadas deben ser mayores a 0.")
    
    if (data['fuel_cost_per_liter'] <= 0).any():
        raise CSVInvalidError("El costo del diésel debe ser mayor a 0.")
    
    if (data['price'] <= 0).any():
        raise CSVInvalidError("Los precios deben ser mayores a 0.")
    
    # 9. Verificar que los equipment_type son válidos (1, 2, 3)
    valid_types = [1, 2, 3]
    invalid_types = data[~data['equipment_type'].isin(valid_types)]['equipment_type'].unique()
    if len(invalid_types) > 0:
        raise CSVInvalidError(
            f"Tipos de equipo inválidos: {list(invalid_types)}. "
...
def train_model_from_csv():
    ...
    try:
        print("📊 [Maquinaria] Iniciando entrenamiento...")
        
        # 1. Validar CSV
        data = validate_csv_file()
        
        # 2. Preparar características
        features = ['equipment_type', 'hours_requested', 'fuel_cost_per_liter']
        
        # Agregar factor de mantenimiento si existe
        if 'mantenimiento_factor' in data.columns:
            features.append('mantenimiento_factor')
            X = data[features]
            print("📊 [Maquinaria] Usando mantenimiento_factor del CSV")
        else:
            # Si no existe, calcular según tipo de equipo
            data['mantenimiento_factor'] = data['equipment_type'].apply(
                lambda x: 1.55 if x == 1 else (1.70 if x == 2 else 1.85)
            )
            features.append('mantenimiento_factor')
            X = data[features]
            print(f"📊 [Maquinaria] Calculando mantenimiento_factor automáticamente")
        
        y = data['price']
        
        # 3. Entrenar el modelo
        print(f"🧠 [Maquinaria] Entrenando modelo con {len(X)} registros...")
        ...
        print(f"✅ [Maquinaria] Modelo entrenado: R²={r2:.4f}, MSE={mse:.4f}")
        
        # 5. Guardar el modelo
        with open(MODEL_FILE, "wb") as f:
            pickle.dump(new_model, f)
        print(f"💾 [Maquinaria] Modelo guardado en {MODEL_FILE}")
        ...
        try:
            ...
            print(f"⚠️ [Maquinaria] No se pudo subir a Supabase: {e}")
        ...
        model_loaded = False
        last_error = str(e)
        print(f"❌ [Maquinaria] Error: {e}")
        raise e
        
    except Exception as e:
        model_loaded = False
        last_error = str(e)
        print(f"❌ [Maquinaria] Error inesperado: {e}")
...
def load_or_train_model():
    ...
    print("🚀 [Maquinaria] Iniciando carga de modelo...")
    ...
    if download_model_from_supabase() and MODEL_FILE.exists():
        try:
            ...
            model_loaded = True
            last_error = None
            print("✅ [Maquinaria] Modelo cargado desde Supabase")
            return True
        except Exception as e:
            print(f"⚠️ [Maquinaria] Error al cargar modelo descargado: {e}")
    ...
    if MODEL_FILE.exists():
        try:
            ...
            model_loaded = True
            last_error = None
            print("✅ [Maquinaria] Modelo cargado desde archivo local")
            return True
        except Exception as e:
            print(f"⚠️ [Maquinaria] Modelo local corrupto: {e}")
    
    # 3. Entrenar desde CSV
    try:
        print("🔄 [Maquinaria] Entrenando modelo desde CSV...")
        result = train_model_from_csv()
        if result['status'] == 'success':
            print("✅ [Maquinaria] Modelo entrenado exitosamente")
            return True
    except Exception as e:
        last_error = str(e)
        print(f"❌ [Maquinaria] Error al entrenar: {e}")
        return False
...
async def quote_maquinaria(data: MaquinariaRequest):
    ...
    try:
        # 2. Validar entrada
        if data.equipment_type not in [1, 2, 3]:
            raise HTTPException(
                status_code=400,
                detail=f"Tipo de equipo inválido: {data.equipment_type}. Tipos válidos: 1, 2, 3"
            )
        
        if data.hours_requested <= 0:
            raise HTTPException(
                status_code=400,
                detail="Las horas solicitadas deben ser mayores a 0"
            )
        
        if data.fuel_cost_per_liter <= 0:
            raise HTTPException(
                status_code=400,
                detail="El costo del diésel debe ser mayor a 0"
            )
        
        # 3. Preparar datos para predicción
        mantenimiento_factor = {
            1: 1.55,
            2: 1.70,
            3: 1.85
        }.get(data.equipment_type, 1.60)
        
        input_df = pd.DataFrame([{
            "equipment_type": data.equipment_type,
            "hours_requested": data.hours_requested,
            "fuel_cost_per_liter": data.fuel_cost_per_liter,
            "mantenimiento_factor": mantenimiento_factor
        }])
        ...
        print(f"❌ [Maquinaria] Error en quote_maquinaria: {e}")
        print(traceback.format_exc())
        raise HTTPException(
            status_code=500,
...
@app.post("/api/maquinaria/retrain")
...
print("=" * 60)
print("🚀 INICIANDO API DE MAQUINARIA (M1)")
print("=" * 60)
...
print("  - POST /api/maquinaria/quote")
print("  - POST /api/maquinaria/retrain")